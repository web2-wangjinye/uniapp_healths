<template name="tabControl">
		<scroll-view  scroll-x="true" :style="'background-color:'+bgc+';top:'+(top?top:customBar)+'px;'" :class="'scrolltab '+(fixed?'uni-fixed':'')" :scroll-left='scrollLeft' scroll-with-animation id="tabcard">
			<view class="tabList" :style="isEqually?'display: flex;justify-content: space-between;padding-left:0;':''" >
				<view
					...
					<text :style="(currentIndex==index?'font-size:'+activeSize+'rpx;color:'+activeColor:'font-size:'+itemSize+'rpx')" :class="boxtype?'box':'line'">{{item.secondTypeDesc}}</text>
					<view :class="'activeLine '+(boxtype?'box':'line')" :style="{width: lineWidth+'%'}"></view>
				</view>
			</view>
		</scroll-view>
</template>
	...
	#tabcard{
		...
		.thisOpenSelect{
			text{
				...
				&.box{
					color: #fff;
    				background: linear-gradient(90deg, #FA7226, #FC2C36);
				}
			}
			.activeLine{
				opacity: 1;
